...
function calibrateWidgetUpdateInterval() {
    ...
    setTimeout(async() => {
        await updateHourlyWidgets(localStorage.getItem('forecastlink'));
        await updateWeeklyWidgets(localStorage.getItem('forecastlink'));
        headerinterval = setInterval(updateWidgets, 3600000);
...
async function updateHourlyWidgets(link) {
    ...
    weatherdata.every(entry => {
        ...
        if ((startTime[0] == d && date.getHours() > parseInt(startTime[1].slice(0,2))) || (total == 0 && startTime[0] != d)) {
        ...
        if (startTime[0] == d || total < 12) {
            ...
            const action = createWeatherWidget([hourlymainwidget, hourlyminorwidget], time, mainwidget, temp, weatherSummary, '', isDaytime);
    ...
    */
    console.log(`Updated Hourly Widgets ${date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
}
...
async function updateWeeklyWidgets(link) {
    ...
    weatherdata.forEach(entry => {
        ...
        const action = createWeatherWidget([weeklymainwidget, weeklyminorwidget], date, mainwidget, temp, weatherSummary, precipitation);
        if (action == 2) {
            mainwidget = true;
        }
    });
    ...
    */
    console.log(`Updated Weekly Widgets ${currdate.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`);
}

function createWeatherWidget(widgettype, date, mainwidget, temp, weatherSummary, precipitation='', isDayTime=true) {
    const divwidget = document.createElement('div');
    const divDT = document.createElement('div');
    const [pDate, pTemp, pSum, pPre] = [document.createElement('p'), document.createElement('p'), document.createElement('p'), document.createElement('p')];
